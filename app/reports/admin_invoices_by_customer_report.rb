class AdminInvoicesByCustomerReport < Ruport::Controller
  ...
  def setup
    ...
    raw_data = LineItem.report_table(:all, 
      ...
      :include=>{ :product=>{:only=>[:ordering_unit,:title,:account_id],:methods=>[:price_in_dollars,:account_name,:estimated_price,:sold_by_weight]}, 
  ...
  formatter :html do
    build :list do
      ...
      output << "* #{SOLD_BY_WEIGHT_EXPLANATION}"
    end
  end
